...
class Submissao:
    ...
    def fit(self, X: torch.Tensor) -> None:
        ...
        d = X.shape[1]
        sigma_lenta = math.sqrt(3*d)
        sigma_media = math.sqrt(d)
        sigma_rapida = math.sqrt(d) * 0.35
        W_lenta = torch.randn(d, 11, generator=g) / sigma_lenta
        W_media = torch.randn(d,10, generator=g) / sigma_media
        W_rapida = torch.randn(d, 10, generator=g) / sigma_rapida
        self.W = torch.cat([W_lenta, W_media, W_rapida], dim=1)

    def phi(self, X: torch.Tensor) -> torch.Tensor:
        Xs = (X - self.mu) / self.sd
        P = (Xs.double() @ self.W.double()).float()
        fourier = torch.cat([torch.sin(P), torch.cos(P)], dim=1)

        # A equação pura da esfera: raio ao quadrado e sua raiz (distância)
        r2 = (Xs ** 2).mean(dim=1, keepdim=True)
        r = r2.sqrt()

        return torch.cat([fourier, r2, r], dim=1)
